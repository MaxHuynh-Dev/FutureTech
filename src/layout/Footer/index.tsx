...
function Footer(): React.ReactElement {
  return (
    <footer className="bg-dark-10" id="footer">
      <Container className="border-dark-15 border-b py-[80px]">
        <div className="grid grid-cols-1 gap-[80px] desktop:grid-cols-5">
              ...
              <div key={col.title} className="flex flex-col gap-[36px]">
                ...
                <div className="flex flex-col gap-[18px]">
                  {col.links.map((l: FooterLinkModel) => {
                    return (
                      <Link key={`${col.title}-${l.label}`} href={l.href} className="flex items-center gap-[8px]">
                        <Text as="span" size={14} color="dark-40" className="desktop:text-[18px] text-dark-40">
                          {l.label}
                        </Text>
                        {l.isNew ? (
                          <span className="bg-dark-10 border-dark-15 inline-flex items-center rounded-[6px] border px-[12px] py-[4px]">
                            <Text as="span" size={12} color="white" className="text-[16px]/[1.5] tracking-[-0.48px]">
                              New
                            </Text>
                          </span>
                        ) : null}
            ...
            <div className="flex flex-col gap-[18px]">
              {FOOTER_RESOURCE_BUTTON_LIST.map((l: FooterLinkModel) => {
                return (
                  ...
                  >
                    {l.label}
                    <ArrowUpRight className="h-5 w-5 text-yellow-55" strokeWidth={1.5} />
                  </Link>
                );
              })}
            </div>
      ...
      <Container className="relative py-[40px]">
        <div className="flex flex-col items-start justify-between gap-[24px] desktop:flex-row desktop:items-center">
          <div className="flex items-center gap-[11px]">
            <Link href="/" className={cn("text-dark-40 text-[18px]/[1.3] tracking-[-0.54px]")}>
              Terms &amp; Conditions
            </Link>
          ...
          <div className="text-dark-40 text-[18px]/[1.3] tracking-[-0.54px]">© 2024 FutureTech. All rights reserved.</div>
          ...
          <div className="desktop:absolute desktop:left-1/2 desktop:top-1/2 desktop:-translate-x-1/2 desktop:-translate-y-1/2 flex items-center gap-[20px]">
            <Link href="/" aria-label="Twitter" className="h-[24px] w-[24px] text-grey-60">
              <span className="sr-only">Twitter</span>
              <ArrowUpRight className="h-6 w-6" strokeWidth={1.25} />
            </Link>
            <Link href="/" aria-label="Dribbble" className="h-[24px] w-[24px] text-grey-60">
              <span className="sr-only">Dribbble</span>
              <ArrowUpRight className="h-6 w-6" strokeWidth={1.25} />
            </Link>
            <Link href="/" aria-label="LinkedIn" className="h-[24px] w-[24px] text-grey-60">
